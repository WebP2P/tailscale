...
type testAttempt struct {
	pkg           string // "github.com/WebP2P/dexnet/types/key"
...
func runTests(ctx context.Context, attempt int, pt *packageTests, goTestArgs, testArgs []string, ch chan<- *testAttempt) error {
	...
	for s.Scan() {
		...
		pkg := cmp.Or(
			goOutput.Package,
			"build:"+goOutput.ImportPath, // can be "./cmd" while Package is "github.com/WebP2P/dexnet/cmd" so use separate namespace
		)
...
func main() {
	...
	if nOfM, ok := strings.CutPrefix(packages[0], "sharded:"); ok && len(packages) == 1 {
		out, err := exec.Command("go", "run", "github.com/WebP2P/dexnet/tool/listpkgs", "-shard", nOfM, "./...").Output()
		if err != nil {
