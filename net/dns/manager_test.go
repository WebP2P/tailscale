...
func TestManager(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			os: OSConfig{
				Nameservers: mustIPs("10.200.0.1"),
			},
			rs: resolver.Config{
				Hosts: hosts(
					"foo.tld.", "1.2.3.4",
		...
		},
		{
			...
			// 9.9.9.9 above), then we don't configure 10.200.0.1 as the
			// resolver.
		...
		},
		{
			...
			// traffic through 10.200.0.1 if there's Split DNS support and the extra
		...
		},
		{
			...
			in: Config{
				DefaultResolvers: mustRes("http://[fd0d:e100:d3c5:ab12:4843:cd96:6245:7a66]:2982/doh"),
				Hosts: hosts(
					"dave.ts.com.", "1.2.3.4",
					"bradfitz.ts.com.", "2.3.4.5"),
				SearchDomains: fqdns("tailscale.com", "universe.tf"),
			...
			rs: resolver.Config{
				Routes: upstreams(".", "http://[fd0d:e100:d3c5:ab12:4843:cd96:6245:7a66]:2982/doh"),
				Hosts: hosts(
					"dave.ts.com.", "1.2.3.4",
					"bradfitz.ts.com.", "2.3.4.5"),
			},
